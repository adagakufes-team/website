export default function Contact() {
  return (
    <section id="contact" className="px-4 py-2 md:px-8 md:py-4">
      <div className="mx-auto max-w-7xl rounded-3xl border-2 border-orange-500 bg-[#f8f8f8] px-6 py-10 text-center md:px-10 md:py-8">
        {/* タイトル */}
        <h2 className="text-2xl font-bold text-gray-900 md:text-3xl">お問い合わせ</h2>

        {/* 下線 */}
        <div className="mx-auto mt-4 h-1 w-24 rounded-full bg-orange-400 md:w-32"></div>

        {/* 本文 */}
        <div className="mt-6 space-y-3 text-base leading-relaxed text-gray-900 md:text-lg">
          <p>メール：adagakufes@gmail.com</p>
          <p>主催：あだち大学フェス実行委員会</p>
        </div>
      </div>
    </section>
  );
}
